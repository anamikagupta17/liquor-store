...
import ProductOne from "../../assetsAdmin/images/product/product-01.png";
...
const TableTwo = ({
  ...
  actionFunction?: (id: string, action: string) => void;
}) => {
  ...
  const SortIcon: React.FC<{ dir: "asc" | "desc" | null }> = ({ dir }) => (
    <span className="ml-1 inline-flex flex-col leading-none">
      {/* up */}
      <svg
        className={`h-[8px] w-[8px] ${dir === "asc" ? "stroke-gray-800" : "stroke-gray-400"}`}
        viewBox="0 0 10 6"
      >
        <path
          d="M1 5L5 1L9 5"
      ...
      {/* down */}
      <svg
        className={`h-[8px] w-[8px] -mt-[1px] ${dir === "desc" ? "stroke-gray-800" : "stroke-gray-400"}`}
        viewBox="0 0 10 6"
      >
        <path
          d="M1 1L5 5L9 1"
  ...
  const handleAction = (productId: string, action: string) => {
    actionFunction(productId, action);
  };
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          <thead>
            <tr className="border-t border-stroke py-4.5 px-4 dark:border-strokedark md:px-6 2xl:px-7.5">
              {columns?.map((item, i) => (
                <th
                  key={i}
                  className="px-4 py-3 text-left w-1/2"
                  onClick={() => toggleSort(item.key)}
                >
                  <p className="font-medium flex items-center gap-1 whitespace-nowrap">
                    {item.name}
                    <SortIcon dir={sortKey === item.key ? sortDir : null} />
                  </p>
                </th>
              ))}
              ...
              >
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center space-x-3.5">
                    <button
                      onClick={() => handleAction(product._id, "edit")}
